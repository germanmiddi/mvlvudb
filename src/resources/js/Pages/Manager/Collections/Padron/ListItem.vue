<template>
    <tr class="hover:bg-gray-50 cursor-pointer">
        ...
        <td
            class="px-6 py-2 whitespace-nowrap text-xs font-medium text-gray-700"
        >
            {{ store.dateFormateada(item.fecha) }}
        </td>
        <td class="px-6 py-2 whitespace-nowrap text-xs text-gray-700">
            <div>{{ item.person }}</div>
        </td>
        <td class="px-6 py-2 whitespace-nowrap text-xs text-gray-700">
        ...
        <td
            class="px-6 py-2 text-center text-sm font-medium flex-col justify-center items-center"
        >
                ...
                <div>
                    <MenuButton class="btn-blue h-7">
                        <EllipsisVerticalIcon
                            ...
                            class="w-7 h-7 inline-flex items-center bg-blue-200 p-1 rounded-full shadow-sm text-gray-900 hover:bg-blue-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                        />
                    </MenuButton>
                </div>
                <transition
...
export default {
    ...
    methods: {
        async changeStatus(newEstado) {
            ...
            // return
            let rt = route("collections.entrevistas.update", {
                id: this.item.id,
            });
