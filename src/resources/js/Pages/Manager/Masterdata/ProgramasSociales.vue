        ...
        <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
            <div>
                <div class="flex justify-between">
                    <h2 id="" class="text-lg leading-6 font-medium text-gray-900">Programas Sociales</h2>
            ...
            <div class="">
                <table class="min-w-full divide-y divide-gray-200">    
                    ...
                    <tbody>
                        <ListItem v-for="t in this.programasSociales" :key="t.id" :item=t 
                                      @edit-item="editItem" 
                                      @hide-item="hideItem" 
...
export default {
    ...
    methods: {
        async getData() {
            let response = await fetch(route('masterdata.programasSociales.get_programaSocial'), { method: 'GET' })
            this.programasSociales = await response.json()
        },
        ...
        async newItem() {
            ...
            try {
                const response = await axios.post(route('masterdata.programasSociales.store_programaSocial'), formData);
        ...
        async editItem(item) {
            ...
            try {
                const response = await axios.post(route('masterdata.programasSociales.update_programaSocial'), formData);
        ...
        async hideItem(id) {
            ...
            const response = await axios.post(route('masterdata.programasSociales.hide_programaSocial'), { id: id });
        ...
        async destroyItem(id) {
            const response = await axios.post(route('masterdata.programasSociales.destroy_programaSocial'), { id: id });
            ...
            if (response.status == 200) {
                ...
                this.programasSociales = this.programasSociales.filter(item => item.id != id)
