...
namespace App\Http\Controllers\Manager\Tramites\Mayor;
...
class MayorController extends Controller
{
    ...
    {
        return Inertia::render('Manager/Tramites/Mayor/Create',
            [
                'paises' => Pais::all(),
                'barrios' => Barrio::all(),
        ...
        try {
            ...
            if($request['tramites_id'] != null){
                ...
                foreach ($request['tramites_id'] as $indice => $valor) {
                    ...
                    Log::info("Se ha almacenado un nuevo tramite", ["Modulo" => "Mayor:store","Usuario" => Auth::user()->id.": ".Auth::user()->name, "ID Tramite" => $tramite_data['id'] ]);
                }
            }else{
                // Se verifica que se haya enviado tipos de tramite
                DB::rollBack();
                Log::error("Por favor ingrese un tipo de tramite", ["Modulo" => "Mayor:store","Usuario" => Auth::user()->id.": ".Auth::user()->name, "Error" => "No se ha ingresado ninguno tramite."]);
            ...
            //dd($th);
            DB::rollBack();
            Log::error("Se ha generado un error al momento de almacenar el tramite", ["Modulo" => "Mayor:store","Usuario" => Auth::user()->id.": ".Auth::user()->name, "Error" => $th->getMessage() ]);
    ...
    {
        return Inertia::render('Manager/Tramites/Mayor/Edit',
            [
                'paises' => Pais::all(),
                'barrios' => Barrio::all(),
                'localidades' => Localidad::all(),
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            Log::info("Se ha actualizado un nuevo tramite", ["Modulo" => "Mayor:update","Usuario" => Auth::user()->id.": ".Auth::user()->name, "ID Tramite" => $request['tramite_id'] ]);
            return response()->json(['message' => 'Se actualizado correctamente el tramite del usuario.', 'idTramite' => $request['tramite_id'] ], 200);
        } catch (\Throwable $th) {
            DB::rollBack();
            Log::error("Se ha generado un error al momento de actualizar el tramite", ["Modulo" => "Mayor:update","Usuario" => Auth::user()->id.": ".Auth::user()->name, "Error" => $th->getMessage() ]);
            return response()->json(['message' => 'Se ha producido un error al momento de actualizar el tramite. Verifique los datos ingresados.'], 203);
        }
    }
    //destroy
    ...
    public function list()
    {
        ...
        $result->where('dependencia_id', 14);
