	...
	<main class="flex-1 bg-gray-50 pb-40">
		...
		<div class="px-4 mt-6 sm:px-6 lg:px-8">
			<form action="#" method="POST">
						...
						<div class="grid grid-cols-12 gap-6">
							...
							<div class="col-span-12 sm:col-span-3">
								<label for="fecha_nac" class="block text-sm font-medium text-gray-700">Fecha de
									Nacimiento</label>
								<Datepicker
									class="mt-1 focus:ring-red-500 focus:border-red-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" :class="input_disable ? bg_disable : ''" :disabled="input_disable"
				...
				<FormNino v-show="!input_disable" :tiposDocumento="tiposDocumento" @data_ninos="ninos_data"></FormNino>

				<FormFamiliar v-show="!input_disable" :parentescos="parentescos" :tiposDocumento="tiposDocumento" :txtFamiliar="'Vinculo con el Menor'"
	...
	methods: {
		...
		async submit() {
			...
			Object.entries(this.form_familiares).forEach(([clave, valor]) => {
				...
				formData.append(
					"familiar_num_documento[]",
					valor.num_documento
				);
				formData.append(
					"familiar_tipo_documento_id[]",
					valor.tipo_documento_id
				);
				formData.append(
					"familiar_fecha_nac[]",
					valor.fecha_nac
						? new Date(valor.fecha_nac).toISOString()
						: null
				);
				formData.append(
			...
			/* Form con datos del niño.*/
			Object.entries(this.form_ninos).forEach(([clave, valor]) => {
				formData.append("ninos_id[]", valor.id);
				formData.append("ninos_name[]", valor.name);
				formData.append("ninos_lastname[]", valor.lastname);
				formData.append("ninos_num_documento[]", valor.num_documento);
				formData.append("ninos_tipo_documento_id[]", valor.tipo_documento_id);
				formData.append("ninos_fecha_nac[]", valor.fecha_nac ? new Date(valor.fecha_nac).toISOString() : null);
			});
