  ...
  <main class="flex-1">
    ...
    >
      ...
      <div>
        ...
        >
          <ArrowPathIcon class="h-5 w-5 text-red-500 animate-spin mr-2" />
          Procesando...
        </a>
      </div>
    </div>
          ...
          <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 sm:col-span-4 lg:col-span-3">
              <label
                for="prueba_estado_id"
                class="block text-sm font-medium text-gray-700"
                >Estado de Pba Pedagógica</label
              >
              <select
            ...
            <p v-if="error" class="text-red-500 text-sm mt-1 w-full">
              {{ error }}
            </p>
          ...
          >
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody v-else class="bg-white divide-y divide-gray-200">
                <tr v-for="data in legajos.data" :key="data.id" class="hover:bg-gray-50">
                  <td class="px-6 py-3 whitespace-nowrap text-xs text-gray-500">

                    <p>{{ data.person?.lastname ?? "-" }}, {{ data.person?.name ?? "-" }}</p>
                    <p class="text-xs text-gray-500">{{ formatNumberWithDots(data.person?.num_documento) ?? "-" }}</p>
                  </td>
                  <td class="hidden px-6 py-3 whitespace-nowrap text-xs text-gray-500">
                    {{ formatNumberWithDots(data.person?.num_documento) ?? "-" }}
                  </td>
                  <td class="px-6 py-3 whitespace-nowrap text-xs text-gray-500">
                  ...
                  <td class="px-6 py-3 whitespace-nowrap text-xs text-gray-500">
                    <span
                      ...
                      >{{ data.estadocbj?.description ?? "-" }}</span
                    >
                    <span
                      v-else
                  ...
                  <td
                    class="px-6 py-3 text-center text-xs font-medium flex justify-center"
                  >
                      ...
                      >
                        ...
                        >
                          <div class="px-1 py-1 text-left">
                            <MenuItem v-slot="{ active }">
                              <a
                                :href="route('legajoCB.legajo', data.id)"
                          ...
                          <div class="px-1 py-1 text-left">
                            <p class="text-xs text-indigo-500 font-bold mt-1">Cambiar Estado:</p>
                            <MenuItem v-slot="{ active }">
                              <a
                                @click="updateEstado(data.id, 2)"
                                class="cursor-pointer block px-4 py-2 text-sm hover:bg-gray-100 text-gray-700"
                              >
                                Regular</a
                              >
                            </MenuItem>
                            ...
                            <MenuItem v-slot="{ active }">
                              <a
                                @click="updateEstado(data.id, 6)"
                                class="cursor-pointer block px-4 py-2 text-sm hover:bg-gray-100 text-gray-700"
                              >
                                Solo en Vacaciones</a
                              >
                            </MenuItem>
                          </div>
                        </MenuItems>
            ...
            <hr />
            <div class="flex justify-between mx-5 my-3 px-2 items-center text-xs">
              <div>
                Mostrando: {{ this.legajos.from }} a {{ this.legajos.to }} -
                Entradas encontradas:
                {{ this.legajos.total }}
              </div>

              <div class="flex flex-wrap -mb-1">
                <div v-for="(link, index) in legajos.links" :key="index">
                  <div
                    v-if="link.url === null"
