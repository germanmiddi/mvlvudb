<template>
                ...
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <div class="grid grid-cols-12 gap-6 m-4">
                        <div class="col-span-12 md:col-span-4 sm:col-span:6">
                            ...
                            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                {{ comment.dependencia.description }}</dd>
                        </div>
                        <div class="col-span-12 md:col-span-4 sm:col-span:6">
                            <dt class="text-sm font-medium leading-6 text-gray-900">Fecha</dt>
                            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                {{ comment.created_at_comment }}</dd>
                        </div>
                        <div class="col-span-12 md:col-span-4 sm:col-span:6">
                            <dt class="text-sm font-medium leading-6 text-gray-900">Responsable</dt>
                            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ comment.user.name }}
                            </dd>
                        </div>
                    </div>
                    <dl class="grid grid-cols-1 gap-x-8 gap-y-10 lg:pt-2 m-4">
                        <div class="flex flex-col items-start">
                            <dd v-if="!editComment" class="mt-2 leading-7 text-gray-600 text-xs text-justify">
                                {{ comment.content }}</dd>
                    ...
                    <dl class="grid grid-cols-1 gap-x-8 gap-y-10 lg:pt-2 m-4">
                        <div v-if="$page.props.user.id === comment.user.id" class="flex justify-end w-full mx-auto">
                            ...
                            <button v-if="!editComment"
                                class="relative inline-flex items-center px-4 py-2 shadow-sm text-xs font-medium rounded-md bg-gray-200 text-gray-900 hover:bg-gray-600 hover:text-white"
                            ...
                            <button v-if="!editComment"
                                class="ml-4 relative inline-flex items-center px-4 py-2 shadow-sm text-xs font-medium rounded-md bg-red-200 text-red-900 hover:bg-red-600 hover:text-white"
    ...
    <DeleteModal v-if="deletedComment" :id="comment.id" :title="`¿Está seguro que desea eliminar el comentario?`"
