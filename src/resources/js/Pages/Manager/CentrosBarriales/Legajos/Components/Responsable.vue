            ...
            <div class="sm:flex sm:justify-between sm:items-center">
                ...
                >
                    <button
                        type="submit"
                        @click="updateLegajo()"
                        class="ml-1 inline-flex items-center px-4 py-1 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                        Guardar
                    </button>
                    <button
                        type="button"
                        @click="cancelEdit()"
                        class="ml-1 inline-flex items-center px-4 py-1 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                        Cancelar
                    </button>
                </div>
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl class="sm:divide-y sm:divide-gray-200">
                    ...
                    >
                        ...
                        <div v-else class="sm:col-span-2">
                            <select
                                ...
                                autocomplete="off"
                                :class="[
                                    input_disable ? bg_disable : 'bg-white',
                                    'block w-full py-2 px-3 border rounded-md shadow-sm focus:outline-none inline-flex focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm',
                    ...
                    >
                        ...
                        <dd
                            v-if="!editData || (hasDni && !changeDni)"
                            class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"
                        >
                            {{ form.num_documento ?? "-" }}
                            <button
                                v-if="hasDni && editData && !changeDni"
                                @click="changeDniMode()"
                        ...
                        <div v-else class="sm:col-span-2 relative">
                            ...
                            <span
                                v-if="v$?.form?.num_documento?.$error"
                    ...
                    >
                        ...
                        <div v-else class="sm:col-span-2">
                            <Datepicker
                                :class="[
                                    'focus:ring-red-500 focus:border-red-500 block w-full shadow-sm sm:text-sm rounded-md',
                                    input_disable ? bg_disable : '',
                    ...
                    >
                        ...
                        <dd
                            ...
                            class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"
                        >
                            {{ this.form.tipo_ocupacion ?? "-" }}
                        </dd>

                        <select
                            v-else
                            v-model="form.tipo_ocupacion_id"
                            id="tipo_ocupacion_id"
                    ...
                    >
                        ...
                        <dd
                            ...
                            class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"
                        >
                            {{ this.form.situacion_conyugal ?? "-" }}
                        </dd>

                        <select
                            v-else
                            v-model="form.situacion_conyugal_id"
                            id="situacion_conyugal_id"
                            name="situacion_conyugal_id"
                            autocomplete="off"
                            :class="input_disable ? bg_disable : ''"
                            :disabled="input_disable"
                            class="sm:col-span-2 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none inline-flex focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        >
                            ...
                            <option
                                v-for="item in situacionesConyugal"
                                :key="item.id"
                                :value="item.id"
                            >
                                {{ item.description }}
                    ...
                    >
                        ...
                        >
                            ...
                            <option
                                v-for="item in nivelesEducativo"
                                :key="item.id"
                                :value="item.id"
                            >
                                {{ item.description }}
                    ...
                    >
                        ...
                        <dd
                            ...
                            class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"
                        >
                            {{ this.form.estado_educativo ?? "-" }}
                        </dd>

                        <select
                            v-else
                            v-model="form.estado_educativo_id"
                            id="estado_educativo_id"
                            name="estado_educativo_id"
                            autocomplete="off"
                            :class="input_disable ? bg_disable : ''"
                            :disabled="input_disable"
                            class="sm:col-span-2 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none inline-flex focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        >
                            ...
                            <option
                                v-for="item in estadosEducativo"
                                :key="item.id"
                                :value="item.id"
                            >
                                {{ item.description }}
...
export default {
    ...
    computed: {
        hasDni() {
            return !!(
                ...
                this.legajo[0].responsable?.num_documento ||
                this.dniStatus
            );
        },
    },
    ...
    methods: {
        initForm() {
            ...
            this.form.num_documento =
                this.legajo[0].responsable?.num_documento ?? null;

            this.form.pais =
                this.legajo[0].responsable?.address[0]?.pais?.description ??
                null;
            this.form.pais_id =
                this.legajo[0].responsable?.address[0]?.pais?.id ?? null;

            this.form.tipo_ocupacion =
                this.legajo[0].responsable?.social[0]?.tipo_ocupacion
                    ?.description ?? null;
            this.form.tipo_ocupacion_id =
                this.legajo[0].responsable?.social[0]?.tipo_ocupacion?.id ??
                null;

            this.form.situacion_conyugal =
                this.legajo[0].responsable?.aditional[0]?.situacion_conyugal
                    ?.description ?? null;
            this.form.situacion_conyugal_id =
                this.legajo[0].responsable?.aditional[0]?.situacion_conyugal
                    ?.id ?? null;

            this.form.tipo_documento =
                this.legajo[0].responsable?.tipo_doc?.description ?? null;
            this.form.tipo_documento_id =
                this.legajo[0].responsable?.tipo_documento_id ?? null;

            this.form.nivel_educativo =
                this.legajo[0].responsable?.education[0]?.nivel_educativo
                    ?.description ?? null;
            this.form.nivel_educativo_id =
                this.legajo[0].responsable?.education[0]?.nivel_educativo?.id ??
                null;

            this.form.estado_educativo =
                this.legajo[0].responsable?.education[0]?.estado_educativo
                    ?.description ?? null;
            ...
            this.form.parentesco =
                this.legajo[0].parentesco?.description ?? null;
            ...
            this.form.fecha_nac = this.legajo[0].responsable?.fecha_nac
                ? typeof this.legajo[0].responsable.fecha_nac === "string"
                    ? new Date(
                          this.legajo[0].responsable.fecha_nac +
                              "T00:00:00.000-03:00"
                      )
                    : this.legajo[0].responsable.fecha_nac
                : null;
            this.form.phone =
                this.legajo[0].responsable?.contact[0]?.phone ?? null;
            this.form.celular =
                this.legajo[0].responsable?.contact[0]?.celular ?? null;
        ...
        handleEditClick() {
            // Si no existe número de documento, mostrar ResponsableNew
            if (!this.form.num_documento) {
                this.showNewForm = true;
            } else {
                // Si existe número de documento, modo edición normal
                this.editData = true;
            }
        },
        changeDniMode() {
        ...
        updateFormWithNewResponsable(responsableData) {
            ...
            this.form.fecha_nac = responsableData.fecha_nac
                ? typeof responsableData.fecha_nac === "string"
                    ? new Date(
                          responsableData.fecha_nac + "T00:00:00.000-03:00"
                      )
                    : responsableData.fecha_nac
                : null;
            ...
            this.form.situacion_conyugal_id =
                responsableData.situacion_conyugal_id;
        ...
        async getPerson() {
            ...
            if (!data.data.length == 0) {
                ...
                this.form.fecha_nac = data.fecha_nac
                    ? new Date(data.fecha_nac + "T00:00:00.000-03:00")
                    : null;
                ...
                this.labelType = "info";
                this.toastMessage =
                    "El DNI indicado no se encuentra registrado";
                this.form.num_documento = num_documento;
        ...
        async updateLegajo() {
            ...
            // Actualiza el Form
            this.form.pais =
                this.paises.find((item) => item.id === this.form.pais_id)
                    ?.description || null;
            this.form.tipo_ocupacion =
                this.tiposOcupacion.find(
                    (item) => item.id === this.form.tipo_ocupacion_id
                )?.description || null;
            this.form.situacion_conyugal =
                this.situacionesConyugal.find(
                    (item) => item.id === this.form.situacion_conyugal_id
                )?.description || null;
            this.form.estado_educativo =
                this.estadosEducativo.find(
                    (item) => item.id === this.form.estado_educativo_id
                )?.description || null;
            this.form.nivel_educativo =
                this.nivelesEducativo.find(
                    (item) => item.id === this.form.nivel_educativo_id
                )?.description || null;
            this.form.parentesco =
                this.parentescos.find(
                    (item) => item.id === this.form.parentesco_id
                )?.description || null;
            this.form.tipo_documento =
                this.tipoDocumento.find(
                    (item) => item.id === this.form.tipo_documento_id
                )?.description || null;
            ...
            try {
                ...
                if (response.status == 200) {
                    ...
                    // Solo salir del modo edición si fue exitoso
                    this.editData = false;
                    this.changeDni = false;
                    ...
                    // Mantener en modo edición si hay error del servidor
                }
            } catch (error) {
                data.message =
