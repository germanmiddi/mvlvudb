                ...
                <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                    <div class="flex items-center justify-between flex-wrap sm:flex-nowrap">
                        ...
                        <div class="flex-shrink-0 flex items-center space-x-2">
                            ...
                            <button type="button"
                                ...
                                @click="getCollectionList()">Aplicar Filtro</button>
                    ...
                    <div class="grid grid-cols-12 gap-6">
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            ...
                            <select v-model="filter.product_id" name="product_id" id="product_id"
                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            <label for="punto_entrega_id" class="block text-sm font-medium text-gray-700">Punto de
                                entrega</label>
                            <select v-model="filter.punto_entrega_id" name="punto_entrega_id" id="punto_entrega_id"
                                ...
                                <option v-for="punto in puntosEntrega" :key="punto.id" :value="punto.id">{{
                                    punto.description }}</option>
                            </select>
                        </div>
                        <div class="col-span-12 sm:col-span-3">
                            <label for="entregado_por" class="block text-sm font-medium text-gray-700">Entregado
                                por</label>
                            <select v-model="filter.entregado_por" name="entregado_por" id="entregado_por"
                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                <option value="" selected>Selecciones un entrevistador</option>
                                <option v-for="entrevistador in users" :key="entrevistador.id"
                ...
                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="item in collections.data" :key="item.id">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{
                                        store.dateFormateada(item.date) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.person }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.num_documento
                                        }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.punto_entrega
                                        }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.entregado_por
                                        }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ item.producto }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <div class="flex justify-between mx-5 my-3 px-2 items-center">
                            <div>
                                Mostrando: {{ this.collections.from }} a {{ this.collections.to }} - Entradas
                                encontradas:
                                {{ this.collections.total }}
                            </div>

                            <div class="flex flex-wrap -mb-1">
                                <template v-for="link in collections.links">
