          ...
          <div class="w-full flex items-center justify-between p-6 space-x-6">
              <div class="flex-1 truncate">
                  <div class="flex items-center space-x-3">
                      <h3 class="text-gray-900 text-base font-medium truncate">{{ informe.programa_social?.description ?? '-' }}</h3>
                      <span
                          class="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">
                          {{informe.estado?.description ?? '-'}}</span>
                  </div>
                  <p class="mt-1 text-gray-500 text-xs truncate">{{ informe.programa_social?.description ?? '-' }}</p>
                  <div class="mt-3">
                      <div class="flex text-sm">
                          <UserCircleIcon class="w-5 text-gray-300"/> <span class="ml-2">{{ store.capitalize(informe.profesional?.name ?? '-') }}</span>
                      </div>
                      <div class="flex text-sm mt-1">
                          <CalendarIcon class="w-5 text-gray-300"/><span class="ml-2">{{store.dateTimeFormateada(informe.fecha_informe ?? '-')}}</span>
                      </div>
                  </div>
              </div>
